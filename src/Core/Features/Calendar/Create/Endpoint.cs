...
public class Endpoint(ILogger<Endpoint> logger, ApplicationDbContext dbContext) : Endpoint<Request, int>
{
    public override async Task HandleAsync(Request req, CancellationToken ct)
    {
        ...
        var calendars = await dbContext.Calendars.Include(calendar => calendar.Year).ToListAsync(cancellationToken: ct);

        if (calendars.Find(x => x.Year.Id.ToString() == req.YearId) != null)
        {
            logger.LogWarning("Calendar for year {YearId} already exists.", req.YearId);
            ThrowError("Calendar for year {YearId} already exists.");
        }
